...
function Benefits() { 
    ...
    <section className={classes.Wrapper}>
        ...
        <h3 className={classes.Title}>EXPERIENCE PRAGUE WITH COOLPASS BENEFITS</h3>
        {benefits.map((el, index) => {
            ...
            return (
                ...
                >{title}</div>
                {(!isActive && index === 0) || (isActive == index ) ? <div className={classes.TextBenefit}>{ convertHtmlToReact(text)  }</div> : null}
                </div>
            )
        })}
