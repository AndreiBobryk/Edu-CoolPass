...
import { setCurrentLanguageGlobal} from "../../../redux/reducer";
...
function MultiLanguage() {
  ...
  function handleClickItemLanguageMenu(e) {
  ...
  return (
    <div className={classes.Wrapper}>
      <button
        ...
        style={{ background: "#252C3E" }}
        onClick={handleClickButton}
      >
        {currentLanguage} <ArrowDown className={classes.Arrow} />
      </button>
      {isOpenListLanguage ? (
          ...
          <div className={classes.List}>
            <ul>
              {Object.keys(objLanguages).map((item, key) => {
                return (
                  <li key={key} onClick={handleClickItemLanguageMenu}>
                    {item}
                  </li>
                );
              })}
